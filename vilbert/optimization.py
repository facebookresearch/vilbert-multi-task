...
def VqaAdam(params, lr=required, data_size=required, batch_size=required, schedule='warmup_step'):
    return WarmupOptimizer(
        lr,
        Optim.Adam(params, betas=(0.9, 0.98), eps=1e-9),
        data_size,
        batch_size,
        schedule)
...
class WarmupOptimizer(object):
    def __init__(self, lr_base, optimizer, data_size, batch_size, schedule):
    ...
    def rate(self, step=None):
        ...
        if self.schedule == 'warmup_linear':
            r = self.lr_base * self._step * self.batch_size / (self.data_size * 2) 
        else:
