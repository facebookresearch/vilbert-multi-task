...
from parallel.scatter_gather import scatter_kwargs, gather
        ...
        if self.use_chuncks:
            ...
            for i in range(1, len(self.device_ids)):
                if i == len(self.device_ids)-1:
                    chunk_sizes[i] = batch_size - sum(chunk_sizes)
                else:
                    chunk_sizes[i] = r_bs
        else:
            chunk_sizes = None

        inputs, kwargs = self.scatter(inputs, kwargs, self.device_ids, chunk_sizes)
        if len(self.device_ids) == 1:
